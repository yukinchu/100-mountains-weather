...
async function showWeather(m) {
  const url = "https://api.open-meteo.com/v1/forecast"
    + "?latitude=" + m.lat
    + "&longitude=" + m.lon
    + "&hourly=temperature_2m,windspeed_10m,precipitation,cloudcover,cloudcover_low,cloudcover_mid,cloudcover_high,weathercode"
    + "&models=jma_seamless"
...
function drawWeatherStrip(times, codes) {
  ...
  for (let i = 0; i < times.length; i += 3) {
    ...
    box.innerHTML = "<div class='wtime'>" + times[i].slice(5,10)
      + "<br>" + times[i].slice(11,16) + "</div>"
